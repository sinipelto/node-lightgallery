...
$(document).ready(() => {
	...
	updateKey = () => {
		...
		fetch("/management", {
			...
			'body': JSON.stringify({
				...
				'data': {
					'usages_init': Number(inputUsageUpdate.value),
					'usages_left': Number(inputUsageUpdate.value)
				}
			})
		})
			.then(respHandler)
		...
		fetch("/management", {
			...
			'body': JSON.stringify({
				...
				'data': {
					...
					'usages_init': Number(inputUsageCreate.value),
					'usages_left': Number(inputUsageCreate.value)
				}
			})
		})
			.then(respHandler)
	...
	infoKey = (tokenId) => {
		window.location.href = `/activity/${tokenId}?key=${AUTH_KEY}`;
	};
});
